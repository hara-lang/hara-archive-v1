...
test("IndexedDB declares the rich HTA Wasm route and complete filesystem export surface", async () => {
  ...
  assert.equal(manifest.namespace, "fs.indexeddb.wasm");
  assert.equal(manifest.identity, "hara/filesystem-indexeddb");
  ...
  assert.deepEqual(manifest.hostCalls["filesystem.indexeddb"], ["describe", "open", "request", "cancel", "close"]);
