...
test("HTA preserves the registry MapEntry frame and rejects wrong arity",async()=>{const suite=parseEdnData(await readFile(new URL("../../../../hara-specs-registry/02-platform/000050-transport-hta/draft/conformance/transport-hta.edn",import.meta.url),"utf8"),"hta/registry-malformed");const field=(map,name)=>{for(const [key,value] of map)if(key instanceof HtaKeyword&&key.name===name)return value;throw new Error(`missing registry field :${name}`);};const fixture=field(suite,"suite/cases").find(candidate=>field(candidate,"case/id").name==="hta.case/map-entry");assert.ok(fixture,"registry MapEntry case");const input=field(fixture,"case/input"),expectation=field(fixture,"case/expect"),expected=Uint8Array.from(field(expectation,"bytes"));const value=new HtaMapEntry(input[0],input[1]);assert.deepEqual(encodeHta(value),expected);const decoded=decodeHta(expected);assert.equal(decoded.constructor,HtaMapEntry);assert.equal(decoded.key.name,"key");assert.equal(decoded.value,42);assert.equal(String(decoded),"[:key 42]");const zero=Uint8Array.from([72,84,65,48,38,0,0,0,0]),one=expected.slice(0,17),three=Uint8Array.from([...expected,0]);one[8]=1;three[8]=3;for(const malformed of [zero,one,three])assert.throws(()=>decodeHta(malformed),/value-malformed: map entry/);});
