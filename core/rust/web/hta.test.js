...
function replaceUtf8(bytes,from,to){const source=new TextEncoder().encode(from),replacement=new TextEncoder().encode(to),copy=Uint8Array.from(bytes);for(let index=0;index<=copy.length-source.length;index++){if(source.every((byte,offset)=>copy[index+offset]===byte)){copy.set(replacement,index);return copy;}}throw new Error(`fixture marker missing: ${from}`);}
