...
import { BrowserPromiseProvider, decodeHta, encodeHta, HtaContext, HtaDeque, HtaHandle, HtaKeyword, HtaMapEntry, HtaPriorityMap, HtaQueue, HtaSortedMap, HtaTagged, HtaSymbol, loadHtaExtension, parseHtaManifest } from "./packages/hta/index.js";
...
test("HTA preserves dedicated map entries under tag 38",()=>{const value=new HtaMapEntry(new HtaKeyword("key"),42);const encoded=encodeHta(value);assert.equal(encoded[4],38);const decoded=decodeHta(encoded);assert.equal(decoded.constructor,HtaMapEntry);assert.equal(decoded.key.name,"key");assert.equal(decoded.value,42);assert.equal(String(decoded),"[:key 42]");});
