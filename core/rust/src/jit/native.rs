...
fn binary(body: &mut Vec<u8>, op: IntrinsicOp) -> Result<(), String> {
    ...
    match op {
        ...
        IntrinsicOp::Modulo => {
            ...
            native_exit(body, -2);
            body.push(0x0b);
            body.extend([0x20, 0x02, 0x20, 0x03, 0x81, 0x21, 0x04]);
            body.extend([0x20, 0x04, 0x50, 0x04, 0x40, 0x05]);
            body.extend([0x20, 0x04, 0x41, 0x00, 0x53]);
            body.extend([0x20, 0x03, 0x41, 0x00, 0x53, 0x72]);
            body.extend([0x04, 0x40]);
            body.extend([0x20, 0x04, 0x20, 0x03, 0x7c, 0x21, 0x04]);
            body.extend([0x0b, 0x0b, 0x20, 0x04]);
        }
        IntrinsicOp::Less
