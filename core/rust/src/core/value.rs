...
fn native_base_provider(native_type: &str, method: &str) -> Result<Value, String> {
    ...
    if method == "apply" {
        return Ok(native_fiber_function(
            &display_name,
            0,
            true,
            {
                let method = method.clone();
                move |arguments| native_base_values(&method, &arguments)
            },
            native_base_apply_fiber,
        ));
    }
...
/// Applies a callable through the continuation-aware boundary used by the
/// direct backend. The ordinary callback above remains the compatibility path
/// for hosts that invoke `Base/apply` synchronously.
fn native_base_apply_fiber(arguments: Vec<Value>, k: Cont) -> Step {
    #[cfg(all(feature = "direct-native", not(target_arch = "wasm32")))]
    if direct_native_execution() {
        if arguments.len() < 2 {
            return k(Err(
                "Base/apply expects a function and a final sequential value".into(),
            ));
        }
        let function = arguments[0].clone();
        let mut call_arguments = arguments[1..arguments.len() - 1].to_vec();
        let values = match iterator_values(arguments.last().cloned().unwrap()) {
            Ok(values) => values,
            Err(error) => return k(Err(error)),
        };
        call_arguments.extend(values);
        return match call_direct_native_fiber(function, call_arguments, k) {
            Ok(step) => step,
            Err(error) => Step::Done(Err(error)),
        };
    }
    k(native_base_values("apply", &arguments))
}
