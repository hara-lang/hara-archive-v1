...
| `selective-intern-in` | `core/lib/src/std/lib/kernel.hal` | `code.vm.kernel` | The retired `std.sandbox` path is not restored. Sandbox lifecycle operations are published as the selected `std.lib.kernel` surface while the native capability boundary remains owned by `code.vm.kernel`. |
